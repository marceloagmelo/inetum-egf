...
export function getDocumentsByCategory(configuration, direction, column) {
  const getFunc = (authParams) => {
    //const url = `${baseUrl}`
    const url =
    //ternário
    direction === "asc"
        ? `https://42105e4d-72dd-4fee-9e5a-0c51ffc7bc99.mock.pstmn.io/documentsSortBy${column}Asc`
        : `https://42105e4d-72dd-4fee-9e5a-0c51ffc7bc99.mock.pstmn.io/documentsSortBy${column}Desc`;

    return axios.get(url, authParams); // Faz uma requisição HTTP GET à API
  };
